...
{
    HF_USART_Init(USART1 , 115200 , 0);  //debug USART init
}
...
void Board::setLedState(uint8_t led_id, uint8_t operation){
    if ( led_id == 0){
        if(operation == 0){ GPIO_SetBits(GPIOA , GPIO_Pin_6);}
        else if(operation == 1) { GPIO_ResetBits(GPIOA , GPIO_Pin_6);}
        else if(operation == 2) { GPIO_ToggleBits(GPIOA , GPIO_Pin_6);}
    }
    else if(led_id == 1){
        if(operation == 0){ GPIO_SetBits(GPIOA , GPIO_Pin_7);}
        else if(operation == 1) { GPIO_ResetBits(GPIOA , GPIO_Pin_7);}
        else if(operation == 2) { GPIO_ToggleBits(GPIOA , GPIO_Pin_7);}
    }
//    else if(led_id == 2){
...
* History:
***********************************************************************************************************************/
void Board::motorEnableForward(uint8_t motor_id)
{
    if(motor_id == 1 ){
        GPIO_SetBits(GPIOE , GPIO_Pin_8);
        GPIO_ResetBits(GPIOE , GPIO_Pin_10);
...
* History:
***********************************************************************************************************************/
void Board::motorEnableBackward(uint8_t motor_id)
{
    if(motor_id == 1 ){
        GPIO_ResetBits(GPIOE , GPIO_Pin_8);			
...
* History:
***********************************************************************************************************************/
void Board::analogServoInterfaceInit(void)
{			  
		HF_PWMOut_Init(TIM9 , 168-1 , 20000 , 0);   //TIM9 CH1-PE5 CH2-PE6
	
	
	
}

void Board::analogServoSetPWM(uint8_t servo_id , int pwm_value)
{
	if(servo_id == 1){
		if((pwm_value < 1250) || (pwm_value > 1750))
		{HF_PWM_Set_TIM9_CCR1(0);
		}
		else
		{
		HF_PWM_Set_TIM9_CCR1(pwm_value);}
	...
	else if(servo_id == 2)
	{
		...
		{
		HF_PWM_Set_TIM9_CCR2(pwm_value);
		}
